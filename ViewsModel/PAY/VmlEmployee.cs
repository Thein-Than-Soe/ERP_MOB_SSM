...
namespace CS.ERP_MOB.ViewsModel.PAY
{
    public class VmlEmployee : BaseViewModel
    {
        ...
        public JSN_LOAD_EMPLOYEE_RO mJSN_LOAD_EMPLOYEE_RO = new JSN_LOAD_EMPLOYEE_RO();
        ...
        public VmlEmployee()
        {
            ...
            EmployeeLoad = new JSN_LOAD_EMPLOYEE_RO();
        ...
        #region "Data Tab"
        public JSN_LOAD_EMPLOYEE_RO JSN_LOAD_EMPLOYEE_RO =new JSN_LOAD_EMPLOYEE_RO();
        public JSN_LOAD_EMPLOYEE_RO EmployeeLoad
        {
            get { return JSN_LOAD_EMPLOYEE_RO; }
            set { JSN_LOAD_EMPLOYEE_RO = value; NotifyPropertyChanged("EmployeeLoad"); }
        }
            ...
            try
            {
                ...
                if (argDAT_EMPLOYEE_LST != null && argDAT_EMPLOYEE_LST.Count > 0)
                {
                    ...
                    foreach (DAT_EMPLOYEE l_DAT_EMPLOYEE in argDAT_EMPLOYEE_LST)
                    {
                        ...
                        if (l_DAT_EMPLOYEE.ROTypeAsk.Equals("2"))
                        {
                            l_DAT_EMP.Add(l_DAT_EMPLOYEE);
                        }
                        if (l_DAT_EMPLOYEE.ROTypeAsk.Equals("3"))
                        {
            ...
            try
            {
                ...
                if (argKeyword != null && !argKeyword.Equals(""))
                {
                    foreach (DAT_EMPLOYEE l_DAT_EMPLOYEE in mJSN_EMPLOYEE.DAT_EMPLOYEE)
                    {
                        ...
                        if (l_DAT_EMPLOYEE.DesignationName.ToLower().Contains(argKeyword)
                            || l_DAT_EMPLOYEE.EmploymentTypeName.ToLower().Contains(argKeyword)
                               || l_DAT_EMPLOYEE.DateofHire.ToLower().Contains(argKeyword)
        ...
        public async void getEmployee()
        {
            try
            {
                ...
                mResponse = await Pay_Service.ApiCall(mRequest, Pay_Name.wsgetEmployeeRO);
        ...
        public async void loadEmployee()
        {
            try
            {
                ...
                mResponse = await Pay_Service.ApiCall(mRequest, Pay_Name.wsloadEmployee);
                if (mResponse != null || mResponse != "")
                {
                    this.mJSN_LOAD_EMPLOYEE_RO = JsonConvert.DeserializeObject<JSN_LOAD_EMPLOYEE_RO>(mResponse);
                    if (mJSN_LOAD_EMPLOYEE_RO.Message.Code == "7")
                    {
                        this.EmployeeLoad = mJSN_LOAD_EMPLOYEE_RO;
                    }
                    else
                    {
                        MessagingCenter.Send<Application, string>(Application.Current, ApplicationMessage.Message.Alert, this.mJSN_LOAD_EMPLOYEE_RO.Message.Message);
                    }
                }
                else
                {
