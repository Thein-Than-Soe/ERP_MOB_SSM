...
{
    public class VmlSalesBilling : BaseViewModel
    {
        #region "Declaring"
        ...
        #region "Contructor"
        public VmlSalesBilling()
        {
            this.switchDisplayView(DisplayView.Card);
            BillingLoad = new JSN_LOAD_SALE_BILLING();
            BillClosedList = new List<RES_SALE_BILLING>();
            BillActiveList = new List<RES_SALE_BILLING>();
            BillPartialList = new List<RES_SALE_BILLING>();
        }
        #endregion
        ...
        public RES_COMPANY RES_COMPANY
        {
            ...
            set { rES_COMPANY = value; NotifyPropertyChanged("RES_SALE_BILLING"); }
        }
        public RES_SALE_BROWSE RES_SALE_BROWSE
        {
            get { return mRES_SALE_BROWSE; }
            set { mRES_SALE_BROWSE = value; NotifyPropertyChanged("RES_SALE_BILLING"); }
        }


        public List<RES_SALE_BILLING> mBillClosedList;
        public List<RES_SALE_BILLING> BillClosedList
        {
            get { return mBillClosedList; }
            set { mBillClosedList = value; NotifyPropertyChanged("BillClosedList"); }
        }

        public List<RES_SALE_BILLING> mBillActiveList;
        public List<RES_SALE_BILLING> BillActiveList
        {
            get { return mBillActiveList; }
            set { mBillActiveList = value; NotifyPropertyChanged("BillActiveList"); }
        }

        public List<RES_SALE_BILLING> mBillPartialList;
        public List<RES_SALE_BILLING> BillPartialList
        {
            get { return mBillPartialList; }
            set { mBillPartialList = value; NotifyPropertyChanged("BillPartialList"); }
        }
        ...
        public ICommand RefreshCommand
        {
            get
            {
                ...
                {
                    mRefreshCommand = new Command(() => this.getBill());
                }
        ...
        private void bindDataTab(List<RES_SALE_BILLING> argRES_SALE_BILLING_LST)
        {
            try
            {
                ...
                List<RES_SALE_BILLING> l_RES_SALE_BILLING_InACTIVE = new List<RES_SALE_BILLING>();
                if (argRES_SALE_BILLING_LST != null && argRES_SALE_BILLING_LST.Count > 0)
                {
                    ...
                    foreach (RES_SALE_BILLING l_RES_SALE_BILLING in argRES_SALE_BILLING_LST)
                    {
                        ...
                        else if (l_RES_SALE_BILLING.StatusAsk.Equals("9"))
                        {
                            l_RES_SALE_BILLING_InACTIVE.Add(l_RES_SALE_BILLING);
                        }
                    }

                    RES_SALE_BILLING = argRES_SALE_BILLING_LST[0];
                    BillClosedList = argRES_SALE_BILLING_LST;
                    BillActiveList = l_RES_SALE_BILLING_ACTIVE;
                    BillPartialList = l_RES_SALE_BILLING_InACTIVE;
                }
                else
                {
                    BillClosedList = new List<RES_SALE_BILLING>();
            ...
            try
            {
                ...
                if (argKeyword != null && !argKeyword.Equals(""))
                {
                    foreach (RES_SALE_BILLING l_RES_SALE_BILLING in mJSN_SALE_BILL_JUN.RES_SALE_BILLING)
                    {
                        ...
                        if (l_RES_SALE_BILLING.BillingCode.ToLower().Contains(argKeyword)
                            || l_RES_SALE_BILLING.ParentCode.ToLower().Contains(argKeyword)
                ...
                {
                    l_RES_SALE_BILLING_lst = mJSN_SALE_BILL_JUN.RES_SALE_BILLING;// OriginalBillClosedList.GetRange(0, OriginalBillClosedList.Count);
                }
        ...
        public async void getBill()
        {
            try
            {
                ...
                mResponse = await Pos_Service.ApiCall(mRequest, Pos_Name.wsgetSaleBillingJun);
        ...
        public async void saveBill()
        {
            ...
            {
                mRequest = JsonConvert.SerializeObject(mJSN_REQ_SALE_BILL_JUN);
                mResponse = await Pos_Service.ApiCall(mRequest, Pos_Name.wssaveSaleBillingJun);
                if (mResponse != null || mResponse != "")
                {
                    this.mJSN_SALE_BILL_JUN = JsonConvert.DeserializeObject<JSN_SALE_BILL_JUN>(mResponse);
                    if (mJSN_SALE_BILL_JUN.Message.Code == "7")
