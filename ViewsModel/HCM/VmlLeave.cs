...
{
    public class VmlLeave : BaseViewModel
    {
        #region "Declaring"
        ...
        public VmlLeave()
        {
            ...
            LeaveLoad = new JSN_LOAD_EMPLOYEE_LEAVE_TAKEN();
        ...
        public JSN_LOAD_EMPLOYEE_LEAVE_TAKEN LeaveLoad
        {
            get { return JSN_LOAD_EMPLOYEE_LEAVE_TAKEN; }
            set { JSN_LOAD_EMPLOYEE_LEAVE_TAKEN = value; NotifyPropertyChanged("LeaveLoad"); }
        }
        ...
        private void bindDataTab(List<DAT_EMPLOYEE_LEAVE_TAKEN> argDAT_EMPLOYEE_LEAVE_TAKEN_LST)
        {
            try
            {
                ...
                if (argDAT_EMPLOYEE_LEAVE_TAKEN_LST != null && argDAT_EMPLOYEE_LEAVE_TAKEN_LST.Count > 0)
                {
                    ...
                    {
                        l_DAT_EMPLOYEE_LEAVE_TAKEN.SD = Utility.getDateTimeString(l_DAT_EMPLOYEE_LEAVE_TAKEN.SD);
                        l_DAT_EMPLOYEE_LEAVE_TAKEN.ED = Utility.getDateTimeString(l_DAT_EMPLOYEE_LEAVE_TAKEN.ED);

                        if (l_DAT_EMPLOYEE_LEAVE_TAKEN.ApproveStatusAsk.Equals("1"))
                        {
                            l_DAT_EMPLOYEE_LEAVE_TAKEN_SUBMIT.Add(l_DAT_EMPLOYEE_LEAVE_TAKEN);
                        }
                        else if (l_DAT_EMPLOYEE_LEAVE_TAKEN.ApproveStatusAsk.Equals("2"))
                        {
                            l_DAT_EMPLOYEE_LEAVE_TAKEN_APPROVED.Add(l_DAT_EMPLOYEE_LEAVE_TAKEN);
                        }
                        else if (l_DAT_EMPLOYEE_LEAVE_TAKEN.ApproveStatusAsk.Equals("3"))
                        {
                ...
                if (argKeyword != null && !argKeyword.Equals(""))
                {
                    foreach (DAT_EMPLOYEE_LEAVE_TAKEN l_DAT_EMPLOYEE_LEAVE_TAKEN in mJSN_EMPLOYEE_LEAVE_TAKEN.DAT_EMPLOYEE_LEAVE_TAKEN)
                    {
                        ...
                        if (l_DAT_EMPLOYEE_LEAVE_TAKEN.EmployeeName.ToLower().Contains(argKeyword)
                            || l_DAT_EMPLOYEE_LEAVE_TAKEN.LeaveTaken.ToLower().Contains(argKeyword)
                            || l_DAT_EMPLOYEE_LEAVE_TAKEN.LeaveRemaining.ToLower().Contains(argKeyword)
        ...
        public async void loadLeave()
        {
            try
            {
                ...
                if (mResponse != null || mResponse != "")
                {
                    ...
                    {
                        this.LeaveLoad = mJSN_LOAD_EMPLOYEE_LEAVE_TAKEN;
                    ...
                    {
                        MessagingCenter.Send<Application, string>(Application.Current, ApplicationMessage.Message.Alert, this.mJSN_LOAD_EMPLOYEE_LEAVE_TAKEN.Message.Message);
                    }
                }
                else
                {
