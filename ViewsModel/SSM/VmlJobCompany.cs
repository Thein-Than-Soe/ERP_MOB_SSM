    ...
    public class VmlJobCompany : BaseViewModel
    {
...

        {
            try
            {
                ...
                mResponse = await Ssm_Service.ApiCall(mRequest, Ssm_Name.wsgetCompanyDetail);
        ...
        public async Task getJobCompany()
        {
            try
            {
                Utility.openLoader();
                mRequest = JsonConvert.SerializeObject(mJSN_REQ_COMPANY);
                mResponse = await Ssm_Service.ApiCall(mRequest, Ssm_Name.wsgetCompany);
                if (mResponse != null && mResponse != "")
                {
        ...
        public async void saveJobCompany(DAT_APPLICANT_COMPANY_JUN arg_DAT_APPLICANT_COMPANY_JUN)
        {
            try
            {
                ...
                mResponse = await Ssm_Service.ApiCall(mRequest, Ssm_Name.wssaveApplicantCompany);
                if (mResponse != null && mResponse != "")
                {
        ...
        public async void loadJobCompany()
        {
            try
            {
                ...
                mResponse = await Ssm_Service.ApiCall(mRequest, Ssm_Name.wsLoadCompany);
                if (mResponse != null && mResponse != "")
                {
