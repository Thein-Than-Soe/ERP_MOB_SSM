...
namespace CS.ERP_MOB.Views.Frame
{
    ...
    public partial class FrmPromotionCard : ContentView
    {
        public FrmPromotionCard()
        {
            InitializeComponent();
        }
        public FrmPromotionCard(RES_PROMOTION argRES_PROMOTION)
        {
            try
            {
                InitializeComponent();
                ImgSubscriberIcon.Source = SubscriberIconSource = argRES_PROMOTION.SubscriberPhoto; ;
                LblSubscriberName.Text =SubscriberLabel = argRES_PROMOTION.SubscriberName_0_255;
                ImgStockIcon.Source = StockIconSource = argRES_PROMOTION.PromotionPhotoURL;
                LblStockName.Text =StockNameLabel = argRES_PROMOTION.PromotionName_0_255;
                LblNewPrice.Text =NewPriceLabel = float.Parse(argRES_PROMOTION.Price).ToString("0.00");
                LblOldPrice.Text =OldPriceLabel = float.Parse(argRES_PROMOTION.OriginalPrice).ToString("0.00");


                if (float.Parse(argRES_PROMOTION.OriginalPrice)<= 0)
                {
        ...
        public static readonly BindableProperty SubscriberIconSourceProperty =
            BindableProperty.Create(propertyName: nameof(SubscriberIconSource),
                                    ...
                                    declaringType: typeof(FrmPromotionCard),
                                    defaultValue: "icon",
        ...
        private static void SubscriberIconSourceChanged(BindableObject bindable, object oldValue, object newValue)
        {
            try
            {
                FrmPromotionCard ThisControl = (FrmPromotionCard)bindable;
