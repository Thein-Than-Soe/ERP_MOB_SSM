...
namespace CS.ERP_MOB.Views.Frame
{
    ...
    public partial class FrmStockCard : ContentView
    {
        public FrmStockCard()
        {
            InitializeComponent();
        }
        public FrmStockCard(RES_STOCK_HOME argRES_STOCK_HOME)
        {
            try
            {
                InitializeComponent();
                ImgSubscriberIcon.Source = SubscriberIconSource = argRES_STOCK_HOME.SubscriberPhoto; ;
                LblSubscriberName.Text =SubscriberLabel = argRES_STOCK_HOME.SubscriberName_0_255;
                ImgStockIcon.Source = StockIconSource = argRES_STOCK_HOME.PhotoURL;
                LblStockName.Text =StockNameLabel = argRES_STOCK_HOME.StockName_0_255;
                LblNewPrice.Text =NewPriceLabel = float.Parse(argRES_STOCK_HOME.StockRetailPrice).ToString("0.00");
               // LblOldPrice.Text =OldPriceLabel = float.Parse(argRES_STOCK_HOME.OriginalPrice).ToString("0.00");


                //if (float.Parse(argRES_STOCK_HOME.OriginalPrice) <= 0)
                //{
        ...
        public static readonly BindableProperty SubscriberIconSourceProperty =
            BindableProperty.Create(propertyName: nameof(SubscriberIconSource),
                                    ...
                                    declaringType: typeof(FrmStockCard),
                                    defaultValue: "icon",
        ...
        private static void SubscriberIconSourceChanged(BindableObject bindable, object oldValue, object newValue)
        {
            try
            {
                FrmStockCard ThisControl = (FrmStockCard)bindable;
