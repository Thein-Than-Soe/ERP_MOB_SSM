...
namespace CS.ERP_MOB.Views.SYS
{
    ...
    {
        #region "Declaring"
        VmlMyOrder mVmlMyOrder { get; set; }
        #endregion
        #region "Constructor"
        ...
        {
            if (mVmlMyOrder.SaleOrderLst == null || !mVmlMyOrder.SaleOrderLst.Any())
                return;
            ...
            switch (sortBy)
            {
                case "Date":
                    sorted = mVmlMyOrder.IsAscending
                        ? mVmlMyOrder.SaleOrderLst.OrderBy(x => x.Date)
                        : mVmlMyOrder.SaleOrderLst.OrderByDescending(x => x.Date);
                    break;
            ...
            mVmlMyOrder.SaleOrderLst = new ObservableCollection<RES_SALE_BROWSE>(sorted);
            if (mVmlMyOrder.IsCardView)
            {
                collectionView.ItemsSource = mVmlMyOrder.SaleOrderLst;
            }
            else if (mVmlMyOrder.IsListView)
            {
                lstView.ItemsSource = mVmlMyOrder.SaleOrderLst;
            }
            else
            {
                MyGrid.ItemsSource = mVmlMyOrder.SaleOrderLst;
            }
        }
        ...
        private void OnEntryCompleted(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    mVmlMyOrder.searchDataApi(text);
                }
            }
            catch (Exception ex)
            {
        ...
        private void OnCheckAllCheckChanged(object sender, CheckedChangedEventArgs e)
        {
            ...
            for (int i = 0; i < mVmlMyOrder.SaleOrderLst.Count; i++)
            {
                var item = mVmlMyOrder.SaleOrderLst[i];
                item.IsChecked = checkAll ? "1" : "0";

                mVmlMyOrder.SaleOrderLst.RemoveAt(i);
                mVmlMyOrder.SaleOrderLst.Insert(i, item);
            }
        }
        private void Sorting_Tapped(object sender, TappedEventArgs e)
        {
            ...
            {
                // Hide all icons
                foreach (var item in mVmlMyOrder.sortingList)
                    item.ShowIcon = false;
        ...
        {
            mVmlMyOrder.IsDescending = false;
            mVmlMyOrder.IsAscending = true;
        }
        private void Descending_Tapped(object sender, TappedEventArgs e)
        {
            mVmlMyOrder.IsDescending = true;
            mVmlMyOrder.IsAscending = false;
        }
        ...
        private void OnListNGridLongPress(object sender, EventArgs e)
        {
            if (sender is Grid gridItem && gridItem.BindingContext is RES_SALE_BROWSE selectedItem)
            {
                if (selectedItem == null)
                    return;
                int index = mVmlMyOrder.SaleOrderLst.IndexOf(selectedItem);
                selectedItem.IsChecked = "1";
                mVmlMyOrder.SaleOrderLst.RemoveAt(index);
                mVmlMyOrder.SaleOrderLst.Insert(index, selectedItem);
