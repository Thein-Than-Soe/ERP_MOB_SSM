...
namespace CS.ERP_MOB.Views.SYS
{
    ...
    {
        #region "Declaring"
        VmlMyTransaction mVmlMyTransaction;
        #endregion
        #region "Constructor"
        public FrmSysMyTransactionLst()
        {
            try
            {
                ...
                mVmlMyTransaction.mJSN_REQ_SALE_LOAD.REQ_AUTHORIZATION = Common.mCommon.REQ_AUTHORIZATION;
                mVmlMyTransaction.mJSN_REQ_SALE_LOAD.RES_SALE_BROWSE = new RES_SALE_BROWSE();
                //  mVmlMyTransaction.mJSN_REQ_SALE_LOAD = new RES_SALE_BROWSE();
                mVmlMyTransaction.getMyTransaction();
                switchLanguage();
            }
            catch (Exception ex)
            {
                    ...
                    case "1"://English
                        {
                            ...
                            TabActive.BadgeText ="Active (" + mVmlMyTransaction.MyTransactionActiveList.Count + ")";
                            TabPartial.BadgeText ="Partial (" + mVmlMyTransaction.MyTransactionPartialList.Count + ")";
                            TabClosed.BadgeText ="Closed (" + mVmlMyTransaction.MyTransactionClosedList.Count + ")";
                        }
                        break;
                    case "2"://Myanmar
                        {
                            entSearch.Placeholder = "ရှာဖွေရန်";
                            TabActive.BadgeText ="အသုံးပြု (" + mVmlMyTransaction.MyTransactionActiveList.Count + ")";
                            TabPartial.BadgeText ="တစိတ်တပိုင်း (" + mVmlMyTransaction.MyTransactionPartialList.Count + ")";
                            TabClosed.BadgeText ="ပိတ်သိမ်း (" + mVmlMyTransaction.MyTransactionClosedList.Count + ")";

                            ACode.HeaderText ="ကုဒ်";
                            AAmount.HeaderText ="ပမာဏ";
                            ADate.HeaderText ="ရက်စွဲ";
                            AType.HeaderText ="ငွေပေးချေမှုအမျိုးအစား";
        ...
        private void entSearch_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                ...
                {
                    mVmlMyTransaction.searchData(e.NewTextValue);
                }
                else
                {
        ...
        private void TgrDisplayColumn_Tapped(object sender, EventArgs e)
        {
            try
            {
                //mVmlMyTransaction.GetCountryData();
            }
            catch (Exception ex)
            {
                throw ex.InnerException;
